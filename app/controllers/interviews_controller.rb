...
#{currentUser.name}
	...
	def new
		# Get the application in question
		application = Application.find(params[:application_id])
		...
		redirect_to [:edit, @source.group, @source, application, interview]
	end
	...
	def destroy
		if Interview.find(params[:id]).destroy
			redirect_to :back, :notice => "Interview cancellation was successful"
		else
			redirect_to :back, :notice => "Interview cancellation was unsuccessful"
		end
	end
