	...
	def favorite
		...
		if fav.nil?
			...
			if fav.save
				...
				Notification.create(:user_id => w.user_id, :notifiable_type => fav.tag!, :message => "{triggered.link} favorited a post of yours.", :link => '')
			else
	...
	def comment
		...
		if comment.save
			...
			if self.current_user.id != whiteboard.user_id
				Notification.create(:notifiable_type => comment.tag!, :user_id => whiteboard.user_id, :message => "{triggered.link} commented on a item you shared.", :link => '')
